...
for i in range(traintime):
    ...
    if i % 50 == 0:
        ...
        np.save("H1_no_noise_loss.npy",Loss)
        print("current L2 error of u is: ", error_u.detach())
        np.save("H1_no_noise_relative_error_u.npy", error_save_u)
        print("current L2 error of f is: ", error_f.detach())
        np.save("H1_no_noise_relative_error_f.npy", error_save_f)
    if i == traintime - 1:
        #save entire model
        torch.save(model, 'H1_no_noise_u_net.pth') 
        torch.save(Net_f, 'H1_no_noise_f_net.pth')
        # save model parameters   
        torch.save(model.state_dict(), 'H1_no_noise_u_net_params.pkl')
        torch.save(Net_f.state_dict(), 'H1_no_noise_f_net_params.pkl')
